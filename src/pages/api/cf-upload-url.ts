 ...
 *
 * Duplicate/official-match detection is handled in /api/upload-cover (server-side)
 * so we always return an upload URL here. The cover will be placed under review
 * by upload-cover if a match is found.
 *
 * Returns: { ok: true, uploadUrl: string, cfImageId: string }
 */
